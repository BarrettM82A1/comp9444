...
class LinearModel:
    ...
    def activation(self, x):
        ...
        if x.dtype != np.float:
            raise ValueError("Input tensor is not a numpy float ndarray!")
    ...
    def forward(self, inputs):
        ...
        res = self.activation(np.dot(self.weights[:-1], inputs) + self.weights[-1])
    ...
    def backward(self, inputs, diff):
        ...
        dw = -np.dot(inputs[:-1], diff.T)
        
        db = -np.sum(diff)
        
        self.weights[:-1] = self.weights[:-1] - self.lr * dw
        self.weights[-1] = self.weights[-1] - self.lr * db        
